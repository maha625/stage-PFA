...
class HotelSearchWizardHotelbeds(models.Model):
    _inherit = 'hotel.search.wizard'

    def _get_hotelbeds_headers(self):
        ...
        if not api_key or not shared_secret:
            raise UserError("Veuillez renseigner l'API Key et le Shared Secret Hotelbeds dans les paramètres GDS.")
    ...
    def _search_hotelbeds(self):
        self.ensure_one()
        self.flush_recordset()
        headers = self._get_hotelbeds_headers()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._format_hotelbeds_results(hotels_data, currency, resolved_code)
            else:
    ...
    def _format_hotelbeds_results(self, hotels_data, currency, resolved_code):
