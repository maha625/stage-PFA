...
class TravelAirlineBlock(models.Model):
    ...
    passenger_line_ids = fields.One2many('travel.rooming.line', 'block_id', string='Rooming List (Passagers)')

    @api.depends('total_seats', 'passenger_line_ids.state')
    def _compute_seats_status(self):
        for record in self:
            sold = len(record.passenger_line_ids.filtered(lambda p: p.state == 'sold'))
            option = len(record.passenger_line_ids.filtered(lambda p: p.state == 'option'))
...
class TravelRoomingLine(models.Model):
    ...
    state = fields.Selection([
        ('option', 'En Option'),
        ('sold', 'Vendu / Confirmé')
    ], string='Statut Siège', default='option', required=True)